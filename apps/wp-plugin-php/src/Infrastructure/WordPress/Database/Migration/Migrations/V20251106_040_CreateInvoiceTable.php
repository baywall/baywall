	...
	public function up(): void {
		...
		$sql = <<<SQL
			CREATE TABLE `{$this->table_name}` (
				...
				CONSTRAINT `chk_{$this->table_name}_payment_token_address` CHECK (`payment_token_address` REGEXP BINARY '^0x[0-9a-f]{40}$'),
				CONSTRAINT `chk_{$this->table_name}_buyer_address` CHECK (`buyer_address` REGEXP BINARY '^0x[0-9a-f]{40}$'),
				PRIMARY KEY (`id`),
