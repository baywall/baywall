...
class UserAccessChecker {
	...
	public function checkCanViewPost( PostId $post_id ): void {
		if ( ! $this->user_access_provider->canViewPost( $post_id ) ) {
			throw new UnauthorizedException( "[E14DA20F] You do not have permission to view this post. post_id: {$post_id}" );
		}
	}
	...
	// throw new UnauthorizedException( "[28E55890] You do not have permission to edit this post. post_id: {$post_id}" );
	// }
	// }
	...
	public function checkHasAdminRole(): void {
		if ( ! $this->user_access_provider->hasAdminRole() ) {
			throw new UnauthorizedException( '[E61EB62A] You do not have admin access.' );
		}
	}
	...
	public function checkCanCreatePost(): void {
		if ( ! $this->user_access_provider->canCreatePost() ) {
			throw new UnauthorizedException( '[6332E030] You do not have permission to create a post.' );
		}
	}
}
